...
AI-Lib 生态系统建立在清晰的三层架构之上，每一层都有明确的职责。当前版本：**AI-Protocol v0.8.4**（`@ailib-official/ai-protocol`）、**ai-lib-rust v0.9.6**、**ai-lib-python v0.8.5**、**ai-lib-ts v0.5.3**、**ai-lib-go v0.6.0**、**ai-protocol-mock v0.1.11**。
...
- **供应商清单**（`providers/*.yaml`）— 37 供应商（10 个 V2 + 27 个 V1）的端点、认证、参数映射、流式解码器、错误分类
...
| Events    | `StreamingEvent` enum | `StreamingEvent` class | 统一流式事件          | `StreamingEvent` struct |
...
- **[AI-Protocol 概览](/zh-cn/protocol/overview/)** — 深入了解规范
- **[Rust SDK](/zh-cn/rust/overview/)** — 探索 Rust 运行时
- **[Python SDK](/zh-cn/python/overview/)** — 探索 Python 运行时
- **[TypeScript SDK](/zh-cn/ts/overview/)** — 探索 TypeScript 运行时
- **[Go SDK](/zh-cn/go/overview/)** — 探索 Go 运行时
