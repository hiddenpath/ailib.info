...
`ai-protocol-mock` は、統合テスト向けに非同期動画生成ライフサイクルを提供します：

- `POST /v1/video/generations`（`{"async": true}`）は `202` と `job_id` を返します
- `GET /v1/video/generations/{job_id}` は `queued -> running -> succeeded` で遷移します
- 失敗注入ヘッダーに対応：
  - `X-Mock-Status`
  - `X-Mock-Timeout-Ms`
  - `X-Mock-Invalid-Content-Type`

これにより、実プロバイダー接続前にポーリング、タイムアウト、終端状態の解析を決定的に検証できます。
